...
class YOLOAugmentor:
    def __init__(self,
        ...
        self.shear = shear
        # 样本增强结果，存储结构
        self.augment_sample={"image":None,
                             "label":None}
        self.current_create_sample_index = 0

        self.img_files=[]
        if not self.is_exist_label:
            self.load_image_paths(img_dir, label_dir)
        elif self.is_exist_label:
            self.load_exist_label_image_paths(img_dir, label_dir)

        self.random_image_index=None
        self.creat_random_image_index()
    ...
    # 只处理，所有带标签的样本集
    def load_exist_label_image_paths(self,img_dir,label_dir):
        total_image_path = sorted(os.listdir(img_dir))
        total_label_name = sorted(os.listdir(label_dir))
        # print(total_image_path)
        self.label_files = [
            os.path.join(label_dir, x)
            for x in total_label_name
        ]
        # print(self.label_files)
        # print(self.img_files)
        for single_label in total_label_name:
            image_name = single_label.replace(".txt",".jpg")
            if image_name in total_image_path:
                # print(image_name)
                self.img_files.append(os.path.join(img_dir,image_name))
        # print(self.img_files)

    def creat_random_image_index(self,):
        index_list = [i for i in range(len(self.img_files))]
        self.random_image_index = random.choice(index_list)
    ...
    def save_augment_sample(self):
        save_sample_image_path = os.path.join(self.output_dir,"images")
        save_sample_label_path = os.path.join(self.output_dir,"labels")
        #创建存储目录
        os.makedirs(save_sample_image_path, exist_ok=True)
        ...
        self.current_create_sample_index += 1

    def apply_pipeline(self):
        """完整增强流程"""
        self.creat_random_image_index()    # 获取随机样本索引
        if random.random() < self.mosaic_prob:
            image, label = self.mosaic_augment_fixed(self.random_image_index)
        else:
            image, label = self.load_image(self.random_image_index)
        ...
        self.augment_sample['image'] = image
        self.augment_sample['label'] = label
        self.save_augment_sample()
        return image, label

# ----------------------------- Example Usage -----------------------------
if __name__ == '__main__':

    augmentor = YOLOAugmentor(img_dir="/home/chenkejing/database/HandDetect/EmdoorRealHandImages/unshare_images/train/images",
