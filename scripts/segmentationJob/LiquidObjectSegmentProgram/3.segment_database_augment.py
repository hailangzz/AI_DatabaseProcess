...
class YOLOSegAugmentor:
    ...
    def __init__(
        ...
        # 获取所有图像文件
        self.img_files = sorted(glob(os.path.join(label_dir, "*.txt")))
    ...
    def random_brightness_contrast(
        self, img, brightness=0.1, contrast=0.15
    ...
    def random_hsv(self, img):
        ...
        if random.random() > self.hsv_prob:
            return img
        r = np.random.uniform(-1, 1, 3) * np.array(
            [self.h_gain, self.s_gain, self.v_gain]
        )
        h, s, v = r
    ...
    def cutout_with_mask_raster_safe(
        ...
        for _ in range(num_holes):
            for _try in range(max_try):
                ...
                # 图像 cutout
                img[y : y + ch, x : x + cw] = np.random.randint(
                    0, 255, (ch, cw, 3), dtype=np.uint8
                )
                break
        ...
        for obj in objects:
            ...
            for i, cnt in enumerate(contours):
                ...
                if hierarchy[0][i][3] != -1:
                    continue
                if len(cnt) >= 3:
                    new_objects.append({"cls": obj["cls"], "poly": cnt.reshape(-1, 2)})
    ...
    def random_rotate(self, img, objects):
        ...
        rotated_img = cv2.warpAffine(
            img, M, (new_w, new_h), flags=cv2.INTER_LINEAR, borderValue=(114, 114, 114)
        )
        ...
        for obj in objects:
            ...
            new_objects.append({"cls": obj["cls"], "poly": rotated_pts})
...
if __name__ == "__main__":
    augmentor = YOLOSegAugmentor(
        img_dir="/data/database/AITotal_Real_Customer_Database/Real_Liquid_Customer_Database/date0616_1/images",
        label_dir="/data/database/AITotal_Real_Customer_Database/Real_Liquid_Customer_Database/date0616_1/yolov8_labels/seg",
        output_dir="/data/database/AITotal_Real_Customer_Database/Real_Liquid_Customer_Database/date0616_1/segment_database_augmentor_0617_batch_1",
        batch_name="segment_real_liquid_seg_0617_batch1",
        augment_ratio=1.0,
    )
