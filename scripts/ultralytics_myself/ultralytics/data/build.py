...
from ultralytics.utils.dataset_meta import DatasetInfo
...
from ultralytics.data.dataset import ConcatDataset
...
def build_yolo_dataset(
    ...
    return dataset(
        ...
        dataset_meta=dataset_meta,  # ⭐新增
        dataset_id=dataset_id,      # ⭐新增
    )
...
def build_dataloader(
    ...
    # print(type(dataset))
    # print(hasattr(dataset, "collate_fn"))
    # print(getattr(dataset, "collate_fn", None))
...
class MultiYOLODataset(ConcatDataset):

    def __init__(self, datasets):
        super().__init__(datasets)

        # 保持与 YOLODataset 尽量一致
        self.rect = any(getattr(ds, "rect", False) for ds in datasets)
...
def build_multi_dataset(
    ...
    for info in dataset_meta.datasets.values():

        ds = build_yolo_dataset(
            cfg=cfg,
            img_path=info.train if mode == "train" else info.val,
            batch=batch,
            data=info.to_data_dict(),
            mode=mode,
    ...
    return MultiYOLODataset(datasets)