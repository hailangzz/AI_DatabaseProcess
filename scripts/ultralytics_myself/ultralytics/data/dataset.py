...
from ultralytics.utils.dataset_meta import DatasetMeta
...
class YOLODataset(BaseDataset):
        ...
        with ThreadPool(NUM_THREADS) as pool:
            ...
            for im_file, lb, shape, segments, keypoint, nm_f, nf_f, ne_f, nc_f, msg in pbar:
                ...
                if im_file:
                    ...
                    # -------------------------------
                    if self.dataset_meta is not None and len(lb):
                        mapper = self.dataset_meta.get_class_map(self.dataset_id)
                        global_cls = []
                        for c in lb[:, 0]:
                            global_cls.append(mapper[int(c)])
                        lb[:, 0] = np.asarray(global_cls, dtype=np.float32)
    ...
    def update_labels_info(self, label: dict) -> dict:
        ...
        label["dataset_id"] = self.dataset_id
        return label
            ...
            x["labels"].append(
                {
                    ...
                    "texts": texts,
                    "dataset_id": self.dataset_id,
                    "class_mask": self.dataset_meta.get_class_mask(self.dataset_id).clone(),
                }
            )
