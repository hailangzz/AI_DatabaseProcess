...
def check_det_dataset(dataset: str, autodownload: bool = True) -> dict[str, Any]:
    ...
    cross_dataset = data.get("cross_dataset", False)
    ...
    for k in ("train", "val"):
        if k not in data:
            ...
            LOGGER.warning("renaming data YAML 'validation' key to 'val'")
    ...
    if not cross_dataset:
        ...
        if val:
            ...
            if not all(x.exists() for x in val):
                ...
                m = (
                    f"Dataset '{name}' images not found, "
                    f"missing path "
                    f"'{next(x for x in val if not x.exists())}'"
                )
                ...
                if s and autodownload:
                    ...
                    m += (
                        f"\nNote dataset download directory "
                        f"is '{DATASETS_DIR}'. "
                        f"You can update this in '{SETTINGS_FILE}'"
                    )
    ...
    if "multi_datasets" not in data:
        ...
        raise SyntaxError(
            "cross_dataset=True but missing 'multi_datasets'"
        )

    datasets = data["multi_datasets"]

    if len(datasets) == 0:

        raise ValueError("multi_datasets is empty")

    for ds in datasets:

        if "name" not in ds:
            raise SyntaxError("dataset missing 'name'")

        if "train" not in ds:
            raise SyntaxError(f"{ds['name']} missing train")

        if "val" not in ds:
            raise SyntaxError(f"{ds['name']} missing val")

        if "data" not in ds:
            raise SyntaxError(f"{ds['name']} missing data")

        if "class_mask" not in ds["data"]:
            raise SyntaxError(
                f"{ds['name']} missing class_mask"
            )

        if "class_map" not in ds["data"]:
            raise SyntaxError(
                f"{ds['name']} missing class_map"
            )

        train_path = Path(ds["train"]).resolve()
        val_path = Path(ds["val"]).resolve()

        if not train_path.exists():

            raise FileNotFoundError(
                f"{ds['name']} train path not found:\n"
                f"{train_path}"
            )
        ...
        if not val_path.exists():
            ...
            raise FileNotFoundError(
                f"{ds['name']} val path not found:\n"
                f"{val_path}"
            )

        ds["train"] = str(train_path)
        ds["val"] = str(val_path)

    LOGGER.info(
        f"Cross Dataset Mode Enabled "
