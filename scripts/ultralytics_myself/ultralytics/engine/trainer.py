...
class BaseTrainer:
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        ...
        with torch_distributed_zero_first(LOCAL_RANK):  # avoid auto-downloading dataset multiple times
            ...
            if isinstance(self.data, dict) and self.data.get("multi_datasets", None):
                self.dataset_meta = DatasetMeta(self.args.data)

                # 更新全局类别
                self.data["names"] = self.dataset_meta.global_names
                self.data["nc"] = self.dataset_meta.global_nc
