...
def extract_ground_mask_y_scaled_rgba(image_path, mask_path, output_path, out_size=640):
    # 读取原图
    ...
    img_rgba[:, :, 3] = mask_full  # alpha 通道用 mask

    # 裁剪 mask 包围框
    ys, xs = np.where(mask_full > 0)
    y_min, y_max = ys.min(), ys.max()
    x_min, x_max = xs.min(), xs.max()
    crop = img_rgba[y_min:y_max+1, x_min:x_max+1]
...
if __name__ == "__main__":
    images_dir = "/home/chenkejing/database/WireDatabase/Cable.v3-v3-wosplits.coco/train/imgs"
    ...
    for file in os.listdir(images_dir):
        if file.lower().endswith(('.png', '.jpg', '.jpeg')):
            ...
            if os.path.exists(mask_path):
                extract_ground_mask_y_scaled_rgba(img_path, mask_path, out_path)