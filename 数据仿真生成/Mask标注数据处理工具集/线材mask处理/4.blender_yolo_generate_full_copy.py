...
ground.data.materials.append(
    make_pbr_mat("floor_mat", texture_path=str(random.choice(floor_images)))
)
...
def add_wire(idx, direction=None):
    ...
    loc_y = (
        ...
        if direction == "y"
        else random.uniform(*HORIZ_DIST_RANGE)
    )
    ...
    wire_obj.data.materials.append(
        make_pbr_mat(
            f"wire_mat_{idx}",
            base_color=color,
            roughness=roughness,
...
def render_frame(i):
    ...
    ground.data.materials[0] = make_pbr_mat(
        "floor_mat", texture_path=str(random.choice(floor_images))
    )
    instances = []
