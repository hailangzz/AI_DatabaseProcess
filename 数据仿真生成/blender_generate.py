# blender_generate.py
# 用法（示例）:
# blender -b -P blender_generate.py -- --outdir ./output2 --num 100 --res 640 480
...
def add_wire(idx):
    ...
    bpy.ops.curve.primitive_bezier_curve_add(
        location=(random.uniform(-0.8, 0.8), random.uniform(-0.8, 0.8), 0.01)
    )
...
def add_water_patch(idx):
    bpy.ops.mesh.primitive_circle_add(
        radius=random.uniform(0.02, 0.15),
        location=(random.uniform(-0.8, 0.8), random.uniform(-0.8, 0.8), 0.001),
    )
...
def add_carpet_patch(idx):
    bpy.ops.mesh.primitive_plane_add(
        size=random.uniform(0.1, 0.6),
        location=(random.uniform(-0.8, 0.8), random.uniform(-0.8, 0.8), 0.002),
    )
...
def render_frame(i):
    ...
    for k in range(random.randint(1, 4)):  # wires
        obj = add_wire(obj_id)
        instances.append(
            {"name": obj.name, "type": "wire", "pass_index": obj.pass_index}
        )
        obj_id += 1
    for k in range(random.randint(0, 3)):  # water
        obj = add_water_patch(obj_id)
        instances.append(
            {"name": obj.name, "type": "water", "pass_index": obj.pass_index}
        )
        obj_id += 1
    if random.random() < 0.6:
        obj = add_carpet_patch(obj_id)
        instances.append(
            {"name": obj.name, "type": "carpet", "pass_index": obj.pass_index}
        )
        obj_id += 1
    ...
    scene.camera.rotation_euler = Euler(
        (math.radians(90 - random.uniform(0, 10)), 0, 0), "XYZ"
    )
